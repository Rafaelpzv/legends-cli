...
function printBanner() {
    console.log('\n' + exports.c.title('╔══════════════════════════════════════════════════════════╗'));
    console.log(exports.c.title('║') + '          ' + exports.c.accent('⚔  LEAGUE OF LEGENDS  CLI  STATS  ⚔') + '          ' + exports.c.title('║'));
...
function printLeaderboard(entries, tier, topN = 20) {
    ...
    sorted.forEach((e, i) => {
        ...
        const wrStr = parseFloat(wr) >= 55 ? exports.c.win(wr + '%') : parseFloat(wr) < 45 ? exports.c.red(wr + '%') : exports.c.dim(wr + '%');
...
function printMatchSummary(matches, puuid) {
    ...
    const table = new cli_table3_1.default({ style: { head: [], border: [] } });
    table.push([exports.c.dim('Partidas analisadas'), exports.c.white(String(n))], [exports.c.dim('Vitórias / Derrotas'), `${exports.c.win(String(wins))} / ${exports.c.loss(String(losses))}`], [exports.c.dim('Taxa de vitória'), parseFloat(wr) >= 50 ? exports.c.win(wr + '%') : exports.c.loss(wr + '%')], [exports.c.dim('KDA médio'), kdaRatio(+(totKills / n).toFixed(1), +(totDeaths / n).toFixed(1), +(totAssist / n).toFixed(1))]);
    console.log(table.toString());
}
// ─── Private helpers ──────────────────────────────────────────────────────────
