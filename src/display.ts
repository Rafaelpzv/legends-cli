...
import { Match, MatchParticipant, ChampionMastery, Summoner, LeagueEntry, QUEUE_MAP } from './types';
...
export function printBanner(): void {
  console.log('\n' + c.title('╔══════════════════════════════════════════════════════════╗'));
  console.log(c.title('║') + '          ' + c.accent('⚔  LEAGUE OF LEGENDS  CLI  STATS  ⚔') + '          ' + c.title('║'));
  console.log(c.title('╚══════════════════════════════════════════════════════════╝'));
...
export function printSummonerCard(summoner: Summoner, gameName: string, tagLine: string): void {
...
function kdaRatio(k: number, d: number, a: number): string {
  ...
  const color = ratio >= 5 ? c.gold : ratio >= 3 ? c.green : ratio >= 1.5 ? c.white : c.red;
...
export function printMatches(matches: Match[], puuid: string): void {
  ...
  const table = new Table({
    head,
    style: { head: [], border: [] },
    colAligns: ['right', 'center', 'left', 'left', 'left', 'center', 'center', 'right', 'right'],
  });
...
export function printLeaderboard(entries: LeagueEntry[], tier: string, topN = 20): void {
  ...
  sorted.forEach((e, i) => {
    ...
    const wrStr  = parseFloat(wr) >= 55 ? c.win(wr + '%') : parseFloat(wr) < 45 ? c.red(wr + '%') : c.dim(wr + '%');
    const rankFn = i === 0 ? c.gold : i === 1 ? c.silver : i === 2 ? c.bronze : c.dim;
    const flags  = [
...
export function printMatchSummary(matches: Match[], puuid: string): void {
  ...
  table.push(
    ...
    [c.dim('Vitórias / Derrotas'), `${c.win(String(wins))} / ${c.loss(String(losses))}`],
    [c.dim('Taxa de vitória'),     parseFloat(wr) >= 50 ? c.win(wr + '%') : c.loss(wr + '%')],
    [c.dim('KDA médio'),          kdaRatio(
