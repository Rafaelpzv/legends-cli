...
import { printBanner, clrscr, c } from "./display";
...
function getApiKey(): string {
  ...
  if (!key) {
    ...
    console.log(
      c.dim("  1. Acesse:  ") + c.accent("https://developer.riotgames.com"),
    );
    console.log(c.dim("  2. Faça login com sua conta Riot"));
    console.log(
      c.dim("  3. Copie sua") +
        c.white(" Development API Key") +
        c.dim(" no painel"),
    );
    ...
    console.log(
      "     " +
        c.green("legends-cli") +
        c.white(" RGAPI-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") +
        "\n",
    );
  ...
  const { region } = await inquirer.prompt([
    {
      ...
      choices: (Object.entries(REGION_LABELS) as [Region, string][]).map(
        ([value, name]) => ({
          name,
          value,
        }),
