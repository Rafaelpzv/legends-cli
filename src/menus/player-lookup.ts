...
async function fetchMatches(client: RiotClient, puuid: string, summonerName: string): Promise<void> {
  ...
  while (true) {
    ...
    try {
      const matches = await client.getMatches(puuid, PAGE_SIZE, filter as QueueFilter, page * PAGE_SIZE);
...
async function fetchMasteries(client: RiotClient, puuid: string, summonerName: string): Promise<void> {
  const spinner = ora({ text: `Buscando maestrias de ${summonerName}…`, color: 'cyan' }).start();
  try {
    await loadChampions();
...
export async function menuPlayerLookup(client: RiotClient): Promise<void> {
  const { riotId } = await inquirer.prompt([{
    ...
    validate: (v: string) => v.trim().length > 0 ? true : 'O nome não pode ser vazio.',
  }]);

  const { gameName, tagLine } = parseRiotId(riotId);

  // ── Lookup account ──
  const spinner = ora({ text: `Buscando conta ${gameName}#${tagLine}…`, color: 'cyan' }).start();
  let puuid: string;
  ...
  try {
    ...
    spinner.succeed(`Jogador encontrado: ${c.bold(account.gameName)}#${account.tagLine}`);
  } catch (err) {
